...
useEffect(()=>{
  ...
  return (
    ...
    className="relative bg-white p-10 rounded-xl text-slate-500">
        ...
        rounded-full mt-5">
            <img className="size-6 gap-0 p-0" src={assets.profile_icon} alt="" />
            <input onChange={e => setName(e.target.value)} value={name}
             type="text" className="ouline-none text-sm" placeholder="Full Name" required/>
        </div>}

        <div className="border px-6 py-2 flex items-center gap-2 
        rounded-full mt-4">
            <img src={assets.email_icon} alt="" />
            <input onChange={e => setEmail(e.target.value)} value={email} type="text" className="ouline-none text-sm" placeholder="Email Id" required/>
        </div>

        <div className="border px-6 py-2 flex items-center gap-2 
        rounded-full mt-4">
            <img src={assets.lock_icon} alt="" />
            <input onChange={e => setPassword(e.target.value)} value={password} type="text" className="ouline-none text-sm" placeholder="Password" required/>
        </div>
        ...
        absolute top-5 right-5 cursor-pointer" onClick={()=>setShowLogin(false)}/>
    </motion.form>
